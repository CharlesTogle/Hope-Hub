...
export async function generateStudentExcel (
  ...
  for (const student of students) {
    ...
    const lectureStatuses = lessonKeys
      .map(key => student[key])
      .filter((lectureStatus): lectureStatus is string => lectureStatus !== undefined);
    ...
    quizKeys.forEach(quizKey => {
      ...
      if (quizValue === undefined) {
        row.push('Incomplete');
      } else if (typeof quizValue === 'boolean') {
        row.push(quizValue ? 'Yes' : 'No');
      } else {
        row.push(quizValue);
      }
    });
  ...
  return { data, headers, classCode: classCode ?? '' };
}
...
export function downloadExcel(
  ...
  const { data, classCode } = workbookData;
 ...
 */
export function generateFilename(type: 'class' | 'student', identifier: string): string {
