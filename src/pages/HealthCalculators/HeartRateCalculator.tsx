import { getHeartRate } from '@/services/Calculations';
...
export default function HeartRateCalculator () {
  ...
  const [thrResult, setThrResult] = useState<string[] | null>(null);
  ...
  const handleCalculate = () => {
    ...
    const thr = getHeartRate(Number(age), heartrate);
        ...
        >
          <div className='flex flex-col gap-3'>
            ...
            <CalculatorInput
              label='Heart Rate'
              value={heartrate}
              setValue={(value) => setHeartrate(Number(value))}
            />
          </div>{' '}
        </CalculatorContainer>
              ...
              <tr>
                ...
                <td>
                  <p className='text-left font-content md:text-center text-xs md:text-base'>
                    {thrResult?.map((results) => (
                      <li key={results} className='list-none'>
                        {results}
                      </li>
                    ))}
                  </p>
