...
export default function BMRCalculator () {
  ...
  const [formulaVariant, setFormulaVariant] = useState<BMRFormula>('Mifflin St Jeor');
  const [bodyFat, setBodyFat] = useState(20);
  const [bmrResult, setBmrResult] = useState(0);
  const [maintainingCalories, setMaintainingCalories] = useState(0);
  const [activityLevel, setActivityLevel] = useState<BMRActivityLevel>(
    'Moderate: exercise 4-5 times/week',
  );
  const [weightGain, setWeightGain] = useState({
    'Maintain Weight': 0.0,
    'Mild Weight Gain': 0.0,
    'Weight Gain': 0.0,
    'Extreme Weight Gain': 0.0,
  });
  ...
  }, [height, weight, age, gender, activityLevel, bodyFat, formulaVariant, heightUnit, weightUnit]);
          ...
          <div className='flex flex-col gap-3'>
            ...
            <div className='z-3 bg-white relative'>
              <RadioButton
                ...
                setValue={(value) => setFormulaVariant(value as BMRFormula)}
                text='BMR Estimation Formula'
                showBodyFat={true}
                bodyFatValue={bodyFat}
                onBodyFatChange={(e) => setBodyFat(Number(e.target.value))}
              />
            </div>
            <div className='z-3 bg-white relative'>
              <RadioButton
                ...
                value={activityLevel}
                setValue={(value) => setActivityLevel(value as BMRActivityLevel)}
                text='Activity Level'
                showBodyFat={false}
              />
            </div>
