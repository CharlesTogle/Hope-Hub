...
export function QuizPage() {
  ...
  const identificationAnswerRef = useIdentificationAnswerRef();
  const questions = useQuizQuestions();
  const { quizState, setQuizState } = useQuizContext();
  ...
  const activeQuizState = quizState;
  const currentQuestion = questions[activeQuizState.questionIndex];
  ...
  function onAnswerSelected(
    ...
    if (multipleChoice) {
      ...
      if (answerText.trim().toLowerCase() === currentQuestion.answer.toLowerCase()) {
        isCorrect = true;
      }
    }
    ...
    const nextQuestionIndex = activeQuizState.questionIndex + 1;
    ...
    let newQuizState: QuizState = {
      ...
      score: activeQuizState.score + (isCorrect ? 1 : 0),
      points: activeQuizState.points + pointsEarnedForCurrentQuestion,
      remainingTime: nextQuestionDuration,
      questionsAnswered: [
        ...activeQuizState.questionsAnswered,
        {
    ...
    if (activeQuizState.questionIndex < questions.length) {
      setTimeout(async () => {
  ...
  const { data: leaderboard = [] } = useQuery<LeaderboardEntry[]>({
    ...
    enabled: activeQuizState.status === 'Done',
    staleTime: 0,
  });
  ...
  return (
    <div>
      <AudioPlayer source={audioFile} shouldStop={activeQuizState.status === 'Done'}>
        {isLoading ? (
              ...
              <div className="flex items-start justify-between pt-8">
                <div>
                  <h2 className="font-heading-small text-2xl lg:text-3xl text-primary-blue ">
                    {activeQuizState.status === 'Pending'
                      ? quizId === '0'
                        ? `Quiz PFT`
                ...
                {activeQuizState.status === 'Pending' && (
                  <Timer
                    key={activeQuizState.questionIndex}
                    duration={
                      shouldShowPoints
                        ? remainingTimeRef.current
                        : activeQuizState.remainingTime
                    }
                    color={'red'}
                    onTimerEnd={() => {
                      onAnswerSelected(
...
function LeaderboardName({ user }: { user: LeaderboardEntry }) {
  return (
    ...
    >
      ...
      {questions[index].type === 'identification' ? (
        ...
        <div className="flex flex-col items-baseline gap-y-2 my-3">
          {(questions.find((question) => question.question === questionData.question)
