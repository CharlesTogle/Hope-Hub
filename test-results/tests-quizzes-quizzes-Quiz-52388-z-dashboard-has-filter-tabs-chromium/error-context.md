...
- Name: tests/quizzes/quizzes.spec.js >> Quiz Dashboard >> quiz dashboard has filter tabs
- Location: e2e/tests/quizzes/quizzes.spec.js:24:3
...
Locator: locator('ul').locator('li').filter({ hasText: /^All$/ })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('ul').locator('li').filter({ hasText: /^All$/ })
